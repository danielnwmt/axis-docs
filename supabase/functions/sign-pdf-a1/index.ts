...
async function drawSignatureStamp(pdfDoc: any, position: any, certRow: any, user: any) {
  ...
  const quad = position.pdfQuad;
  const quadPoints = quad ? [quad.tl, quad.tr, quad.br, quad.bl] : [];
  const hasPdfQuad = quadPoints.length === 4 && quadPoints.every((p) => Array.isArray(p) && Number.isFinite(Number(p[0])) && Number.isFinite(Number(p[1])));
  const hasPdfCoords = !hasPdfQuad && [position.pdfX, position.pdfY, position.pdfW, position.pdfH].every((v) => Number.isFinite(Number(v)));
  const xs = hasPdfQuad ? quadPoints.map((p) => Number(p[0])) : [];
  const ys = hasPdfQuad ? quadPoints.map((p) => Number(p[1])) : [];
  let wBox = Math.min(hasPdfQuad ? Math.max(...xs) - Math.min(...xs) : hasPdfCoords ? Number(position.pdfW) : wr * crop.width, crop.width);
  let hBox = Math.min(hasPdfQuad ? Math.max(...ys) - Math.min(...ys) : hasPdfCoords ? Number(position.pdfH) : hr * crop.height, crop.height);
  let x = hasPdfQuad ? Math.min(...xs) : hasPdfCoords ? Number(position.pdfX) : crop.x + xr * crop.width;
  let y = hasPdfQuad ? Math.min(...ys) : hasPdfCoords ? Number(position.pdfY) : crop.y + crop.height - yr * crop.height - hBox;
